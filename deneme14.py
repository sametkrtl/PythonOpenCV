...
# ----------------------------------------------------
#  EnhancedAnonymizationApp  (Custom NER Model + Tutarlı Değiştirme + Karakter Sayısı Koruma)
# ----------------------------------------------------
class EnhancedAnonymizationApp:
    ...
    def generate_text_with_exact_length(self, target_length: int, entity_type: str, seed_text: str, max_attempts: int = 50) -> str:
        """Belirli uzunlukta metin üret (kesme yerine yeni değer dene)"""
        f = self._seeded_faker(seed_text)
        
        for attempt in range(max_attempts):
            # Farklı seed ile yeni değer üret
            attempt_seed = f"{seed_text}_{attempt}"
            f_attempt = self._seeded_faker(attempt_seed)
            
            candidate = ""
            if entity_type == 'ad_soyad':
                candidate = self._fake_name_tr(f_attempt)
            elif entity_type == 'telefon':
                candidate = self._fake_phone_tr(f_attempt)
            elif entity_type == 'email':
                candidate = self._fake_email_from_name(f_attempt, seed_text)
            elif entity_type == 'adres':
                candidate = self._fake_address_tr(f_attempt)
            elif entity_type == 'sirket':
                candidate = self._fake_company_tr(f_attempt)
            elif entity_type == 'iban':
                candidate = self._fake_iban_tr(f_attempt)
            elif entity_type == 'tc_kimlik':
                candidate = self._fake_tc_kimlik(f_attempt)
            elif entity_type == 'para':
                # Para için farklı formatlar dene
                if target_length <= 6:
                    tl = f_attempt.random_int(10, 999)
                    candidate = f"{tl} TL"
                elif target_length <= 10:
                    tl = f_attempt.random_int(1000, 9999)
                    candidate = f"{tl} TL"
                else:
                    tl = f_attempt.pydecimal(left_digits=4, right_digits=2, positive=True)
                    candidate = f"{tl} TL"
            elif entity_type == 'tarih':
                # Farklı tarih formatları dene
                dt = f_attempt.date_between(start_date="-10y", end_date="today")
                if target_length <= 8:
                    candidate = dt.strftime("%d.%m.%y")  # 8 karakter
                elif target_length <= 10:
                    candidate = dt.strftime("%d.%m.%Y")  # 10 karakter
                else:
                    candidate = dt.strftime("%d %B %Y")  # Uzun format
            else:
                # Bilinmeyen tip için rastgele string
                candidate = ''.join(f_attempt.random_choices(
                    'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 
                    length=min(target_length, 20)
                ))
            
            # Eğer tam uzunlukta ise kullan
            if len(candidate) == target_length:
                return candidate
            
            # Eğer çok kısaysa ve padding yapabilirsek (sayılar için)
            if len(candidate) < target_length and entity_type in ['telefon', 'tc_kimlik']:
                if entity_type == 'telefon' and candidate.startswith('05'):
                    # Telefon numarası için eksik rakamları ekle
                    needed = target_length - len(candidate)
                    extra_digits = ''.join(str(f_attempt.random_int(0, 9)) for _ in range(needed))
                    candidate = candidate + extra_digits
                    if len(candidate) == target_length:
                        return candidate
                elif entity_type == 'tc_kimlik':
                    # TC için yeniden üret (11 karakter sabit)
                    continue
        
        # Eğer hiçbiri uygun değilse, orijinal uzunlukta generic metin üret
        generic_chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
        f_final = self._seeded_faker(f"{seed_text}_final")
        return ''.join(f_final.random_choices(generic_chars, length=target_length))
    ...
    def process_pdf_with_real_replacement(self, 
        ...
        """
        Gelişmiş PDF işleme - GERÇEK değiştirme ile (Custom NER + Tutarlı değiştirme + Karakter koruma)
        """
        if pdf_file is None:
    ...
    # ------------ Gelişmiş Faker yardımcıları (Tutarlı + Karakter Sayısı Koruma) ------------
    ...
    def generate_consistent_replacement(self, original_text: str, entity_type: str, strategy: str, custom_replacements: Dict) -> str:
        """Aynı orijinal metin için tutarlı değiştirme üret + tam karakter sayısı koruma"""
        
        # Cache'te varsa aynı değeri döndür
        ...
        if strategy == "Maskeleme (*)":
            ...
            base_replacement = generic_map.get(entity_type, f'[{entity_type.upper()}]')
            
            # Genel değer için tam uzunlukta üret
            if len(base_replacement) == target_length:
                replacement = base_replacement
            elif len(base_replacement) < target_length:
                # Eksik kısmı sayılarla doldur
                needed = target_length - len(base_replacement)
                f = self._seeded_faker(original_text)
                extra = ''.join(str(f.random_int(0, 9)) for _ in range(needed))
                replacement = base_replacement + extra
            else:
                # Çok uzun ise kısalt ama anlamlı kalsın
                replacement = base_replacement[:target_length]
                
        elif strategy == "Sahte Değerler":
            # Özel değer varsa önce onu dene
            if entity_type in custom_replacements:
                custom_value = custom_replacements[entity_type]
                if len(custom_value) == target_length:
                    replacement = custom_value
                elif len(custom_value) < target_length:
                    # Özel değer kısa ise, onu temel alarak uzat
                    needed = target_length - len(custom_value)
                    f = self._seeded_faker(original_text)
                    if entity_type in ['telefon', 'tc_kimlik']:
                        extra = ''.join(str(f.random_int(0, 9)) for _ in range(needed))
                    else:
                        extra = ''.join(f.random_choices('ABCDEFGHIJKLMNOPQRSTUVWXYZ', length=needed))
                    replacement = custom_value + extra
                else:
                    # Özel değer uzun ise tam uzunlukta yeni değer üret
                    replacement = self.generate_text_with_exact_length(target_length, entity_type, original_text)
            else:
                # Faker ile tam uzunlukta değer üret
                replacement = self.generate_text_with_exact_length(target_length, entity_type, original_text)
        else:
    ...
    def apply_replacement_strategy_consistent(self, entities: List[Dict], strategy: str, custom_replacements: str) -> List[Dict]:
...
""".strip()
    ...
    def create_enhanced_interface(self):
        ...
        with gr.Blocks(css=css, title="PDF Kişisel Bilgi Anonimleştirici - Enhanced NER", theme=gr.themes.Soft()) as interface:
            # Model durumu göster
            ...
            gr.Markdown(f"""
            # 🔐 PDF Kişisel Bilgi Anonimleştirici — Enhanced NER
            Yüklediğiniz PDF'teki kişisel bilgileri **özel modelinizle tespit eder** ve **tutarlı + karakter sayısı korumalı değiştirme** yapar.
            
            <div class="model-status {model_class}">
            🤖 **Model Durumu**: {model_status}<br>
            📁 **Model Yolu**: {self.model_path}<br>
            🔧 **Yeni Özellikler**: Tutarlı değiştirme, karakter sayısı koruma, sadece TC kimlik için regex
            </div>
            """)
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("📄 PDF İşleme", elem_id="main-tab"):
                    with gr.Row():
                        with gr.Column(scale=1):
                            ...
                            with gr.Accordion("🧩 Özel Değer Haritası (JSON)", open=False):
                                gr.Markdown("**Not**: Sistem tam karakter sayısı eşleşmesi için Faker'dan farklı değerler deneyecektir.")
                                gr.Markdown("Örnek:")
                                example_json = {
                    ...
                    with gr.TabItem("🧪 Model Test"):
                        gr.Markdown("### Custom NER Model + TC Kimlik Regex Testi")
                        ...
                        def test_model_enhanced(text, conf_threshold):
                    ...
                    with gr.TabItem("🗂️ Geçmiş"):
                        gr.Markdown("Son işlemlerin özeti (tutarlılık bilgileri dahil).")
    ...
    - **Yeni Özellikler**: Tutarlı değiştirme + karakter sayısı koruma

    ### 🔧 Yeni Gelişmiş Özellikler
    - **Tutarlı Değiştirme**: Aynı orijinal metin her zaman aynı değer ile değiştirilir
    - **Tam Karakter Eşleştirme**: Değiştirme metni orijinal ile tam aynı karakter sayısına sahip olur
    - **Akıllı Faker Denemesi**: Uygun uzunluk bulunana kadar farklı Faker değerleri denenir (kesme yok)
    - **Sadece TC Regex**: TC kimlik numaraları dışında tüm tespitler NER model ile yapılır
    - **Cache Sistemi**: Aynı dosya işlenirken tutarlılık için cache kullanılır
    ...
    **NER Model ile:**
    ...
    - E-posta (EMAIL)
    - Adres (ADDRESS)
    - Şirket (ORGANIZATION/ORG)
    - Para (MONEY)
    - Tarih (DATE)

    **Regex ile (sadece):**
    - TC Kimlik Numarası (doğrulama dahil)

    ### 🔍 Tam Karakter Eşleştirme Örnekleri
    ```
    Orijinal: "Ahmet Yılmaz" (12 karakter)
    Deneme 1: "Mehmet Kaya" (11 karakter) → ❌ Reddedildi
    Deneme 2: "Fatma Özdemir" (13 karakter) → ❌ Reddedildi  
    Deneme 3: "Zehra Doğan" (12 karakter) → ✅ Kabul edildi

    Orijinal: "05321234567" (11 karakter)
    Faker değer: "05429876543" (11 karakter) → ✅ Direkt kabul
    ```

    ### 🚨 Önemli Notlar
    1. **Kesme Yok**: "İstanbul" → "İstan" gibi kesme işlemi yapılmaz
    2. **Yıldız Yok**: Eksik karakterler '*' ile doldurulmaz
    3. **Akıllı Deneme**: Maksimum 50 deneme ile uygun uzunlukta değer bulunur
    4. **Anlamlı Sonuç**: Bulunamadığında bile anlamlı generic değer üretilir
    5. **Tutarlılık**: Aynı "Ahmet" metni her yerde aynı uzunlukta aynı değerle değiştirilir
                        """)
